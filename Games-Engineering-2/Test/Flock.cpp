...
void Flock::cFormation(int fLeader)
{
	...
	for (int i = 0; i < fSize; i++)
	{
		...
		if (i == fLeader) {		//Deal with our leader here
			...
			flock[i].velocity.mulScalar(target.maxSpeed / 50);
	...
	for (int i = 0; i < fSize; i++)
	{
		...
		if (i == fLeader) {		//Deal with our leader here
			...
			flock[i].borders();
				...
				angleAroundV = (float)i / (fSize - 1);
				angleAroundV = angleAroundV * pi * 2;
				float radius = npcRadius / sin(pi / (fSize));
				targetSlot = target.location;
				targetSlot.x = targetSlot.x + radius * cos(5) - 300;
				targetSlot.y = targetSlot.y + radius * sin(angleAroundV) - 30;
				sub = sub.subTwoVector(targetSlot, flock[i].location);
				...
				targetSlot = target.location;
				targetSlot.x = targetSlot.x + radius * cos(5) - 300;
				targetSlot.y = targetSlot.y + radius * sin(angleAroundV);
				sub = sub.subTwoVector(targetSlot, flock[i].location);
